...
{      
  ...
  robot_settings = new QSettings;
	...
	geo_vars->addChild(field_vars);
		ADD_VALUE(field_vars,Double,Field_Length,9.450,"Length")
		ADD_VALUE(field_vars,Double,Field_Width,6.32,"Width")
...
#else
	world=VarXML::read(world,(qApp->applicationDirPath() + QString("/settingslarge.xml")).toStdString());
#endif
...
ConfigWidget::~ConfigWidget() {  
#ifndef LARGE_FIELD
   VarXML::write(world,(qApp->applicationDirPath() + QString("/settingslarge.xml")).toStdString());
#else
