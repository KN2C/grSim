...
{    
    ...
    for (int k=0;k<ROBOT_COUNT;k++)
        robots[k] = new Robot(p,ball,cfg,-form1->x[k],form1->y[k],ROBOT_START_Z(cfg),ROBOT_GRAY,ROBOT_GRAY,ROBOT_GRAY,(k<5)?k+1:4,wheeltexid,1);
    cfg->robotSettings = cfg->yellowSettings;
    for (int k=0;k<ROBOT_COUNT;k++)
        robots[k+ROBOT_COUNT] = new Robot(p,ball,cfg,form2->x[k],form2->y[k],ROBOT_START_Z(cfg),ROBOT_GRAY,ROBOT_GRAY,ROBOT_GRAY,(k<5)?k+6:10,wheeltexid,-1);
...
void SSLWorld::glinit()
{
    ...
    g->loadTexture(createBlob('y',0,&robots[6]->img));
    g->loadTexture(createBlob('y',1,&robots[7]->img));
    g->loadTexture(createBlob('y',2,&robots[8]->img));
    g->loadTexture(createBlob('y',3,&robots[9]->img));
    g->loadTexture(createBlob('y',4,&robots[10]->img));
#else
	g->loadTexture(createBlob('b',0,&robots[0]->img));
	g->loadTexture(createBlob('b',1,&robots[1]->img));
	g->loadTexture(createBlob('b',2,&robots[2]->img));
	g->loadTexture(createBlob('b',3,&robots[3]->img));
	g->loadTexture(createBlob('b',4,&robots[4]->img));
	g->loadTexture(createBlob('b',0,&robots[6]->img));
	g->loadTexture(createBlob('b',1,&robots[7]->img));
	g->loadTexture(createBlob('y',0,&robots[8]->img));
	g->loadTexture(createBlob('y',1,&robots[9]->img));
	g->loadTexture(createBlob('y',2,&robots[10]->img));
	robots[11]->img = robots[9]->img;
	robots[12]->img = robots[10]->img;
	robots[13]->img = robots[8]->img;
	robots[14]->img = robots[9]->img;
	robots[15]->img = robots[10]->img;
#endif
    ...
    robots[5]->img = robots[4]->img;
    robots[11]->img = robots[10]->img;
...
RobotsFomation::RobotsFomation(int type)
{
    ...
    if (type==0)
    {
        dReal teamPosX[ROBOT_COUNT] = {2.2, 1.0 , 1.0, 1.0, 0.33, 0.33};
        dReal teamPosY[ROBOT_COUNT] = {0.0, -0.75 , 0.0, 0.75, 0.25, 0.65};
        setAll(teamPosX,teamPosY);
    }
    if (type==1)
    {
        dReal teamPosX[ROBOT_COUNT] = {1.0, 1.0, 1.0, 0.33, 1.7, 0.33};
        dReal teamPosY[ROBOT_COUNT] = {0.75, 0.0, -0.75, -0.25, 0.0, 0.65};
        setAll(teamPosX,teamPosY);
    }
    if (type==2)
    {
        dReal teamPosX[ROBOT_COUNT] = {2.8, 2.5, 2.5, 0.8, 0.8, 0.8};
        dReal teamPosY[ROBOT_COUNT] = {0.0, -0.3, 0.3, 0.0, 1.5, 1.0};
        setAll(teamPosX,teamPosY);
    }
    if (type==3)
    {
        dReal teamPosX[ROBOT_COUNT] = {2.8, 2.5, 2.5, 0.8, 0.8, 0.8};
        dReal teamPosY[ROBOT_COUNT] = {5.0, 5-0.3, 5+0.3, 5+0.0, 5+1.5, 5+1};
        setAll(teamPosX,teamPosY);
    }
    if (type==4)
    {
        dReal teamPosX[ROBOT_COUNT] = {2.8, 2.5, 2.5, 0.8, 0.8, 0.8};
        dReal teamPosY[ROBOT_COUNT] = {5+0.0, 5-0.3, 5+0.3, 5+0.0, 5+1.5, 5+1};
        setAll(teamPosX,teamPosY);
    }
    if (type==-1)
    {
        dReal teamPosX[ROBOT_COUNT] = {-0.8, -0.4, 0, 0.4, 0.8, 1.2};
	...
	if (type==0)
	{
		dReal teamPosX[ROBOT_COUNT] = {2.2, 1.0 , 1.0, 1.0, 0.33, 0.33, 1.5, 1.5};
		dReal teamPosY[ROBOT_COUNT] = {0.0, -0.75 , 0.0, 0.75, 0.25, 0.65, 0.5, -0.5};
		setAll(teamPosX,teamPosY);
	}
	if (type==1)
	{
		dReal teamPosX[ROBOT_COUNT] = {1.0, 1.0, 1.0, 0.33, 1.7, 0.33, 1.5, 1.5};
		dReal teamPosY[ROBOT_COUNT] = {0.75, 0.0, -0.75, -0.25, 0.0, 0.65, 0.5, -0.5};
		setAll(teamPosX,teamPosY);
	}
	if (type==2)
	{
		dReal teamPosX[ROBOT_COUNT] = {2.8, 2.5, 2.5, 0.8, 0.8, 0.8, 1.5, 1.5};
		dReal teamPosY[ROBOT_COUNT] = {0.0, -0.3, 0.3, 0.0, 1.5, 1.0, 0.5, -0.5};
		setAll(teamPosX,teamPosY);
	}
	if (type==3)
	{
		dReal teamPosX[ROBOT_COUNT] = {2.8, 2.5, 2.5, 0.8, 0.8, 0.8, 1.5, 1.5};
		dReal teamPosY[ROBOT_COUNT] = {5.0, 5-0.3, 5+0.3, 5+0.0, 5+1.5, 5+1, 5+0.5, 5-0.5};
		setAll(teamPosX,teamPosY);
	}
	if (type==4)
	{
		dReal teamPosX[ROBOT_COUNT] = {2.8, 2.5, 2.5, 0.8, 0.8, 0.8, 1.5, 1.5};
		dReal teamPosY[ROBOT_COUNT] = {5+0.0, 5-0.3, 5+0.3, 5+0.0, 5+1.5, 5+1, 5+1.5, 5+1.5};
		setAll(teamPosX,teamPosY);
	}
	if (type==-1)
	{
		dReal teamPosX[ROBOT_COUNT] = {-0.8, -0.4, 0, 0.4, 0.8, 1.2, 1.4, 1.6};
		dReal teamPosY[ROBOT_COUNT] = {-2.7,-2.7,-2.7,-2.7,-2.7, -2.7, -2.7, -2.7};
		setAll(teamPosX,teamPosY);
	}
	if (type==-2)
	{
		dReal teamPosX[ROBOT_COUNT] = {-0.8, -0.4, 0, 0.4, 0.8, 1.2, 1.4, 1.6};
		dReal teamPosY[ROBOT_COUNT] = {-2.3,-2.3,-2.3,-2.3,-2.3, -2.3,-2.3, -2.3};
		setAll(teamPosX,teamPosY);
	}
#endif
}
